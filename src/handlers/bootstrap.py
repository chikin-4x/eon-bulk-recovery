...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    # If cross-account role is provided, assume it to get credentials for restore account
    if "crossAccountRoleArn" in event:
        sts_client = boto3.client("sts")
        response = sts_client.assume_role(
            RoleArn=event["crossAccountRoleArn"],
        ...
        # Use default credentials (Lambda execution role must have permissions)
        cfn_client = boto3.client("cloudformation", region_name=restore_region)
        rds_client = boto3.client("rds", region_name=restore_region)
        kms_client = boto3.client("kms", region_name=restore_region)
